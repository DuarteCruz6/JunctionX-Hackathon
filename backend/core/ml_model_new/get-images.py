...
import math
import os
from concurrent.futures import ProcessPoolExecutor, as_completed # IMPORTANT: Switched to ProcessPoolExecutor

# --- User-defined variables (change these) ---
SHAPEFILE_PATH = 'C:/Users/danim/OneDrive/Ambiente de Trabalho/COS2018v2-S1-shp/COS2018v2-S1.shp'
ATTRIBUTE_COLUMN = 'COS18n4_C'
TARGET_VALUE = '5.1.1.6'

OUTPUT_DIR = 'C:/Users/danim/OneDrive/Ambiente de Trabalho/dataset/train/ctgry_1'
OFFSET_FACTOR = 0.1
MAX_FRAME_SIZE_DEGREES = 0.5
DPI = 200
# For multiprocessing, it's best to use a number close to your CPU cores.
# os.cpu_count() can find this automatically, or you can set it manually.
...
# This function is now run in a completely separate process.
def process_frame(frame_data):
    feature_id, i_frame, f_minx, f_miny, f_maxx, f_maxy, crs_str = frame_data

    # Each process creates its own figure and axes
    ...
    try:
        ...
        # We return an error message instead of just printing it
...
# IMPORTANT: This `if` block is required for multiprocessing to work safely
if __name__ == "__main__":
    ...
    print(f"Filtering for areas where '{ATTRIBUTE_COLUMN}' is '{TARGET_VALUE}'...")
    target_areas = world[world[ATTRIBUTE_COLUMN] == TARGET_VALUE]

    if target_areas.empty:
        print("Warning: No areas found with the specified attribute value.")
    else:
        ...
        for index, row in target_areas.iterrows():
            ...
            for i_frame, (f_minx, f_miny, f_maxx, f_maxy) in enumerate(frames_for_feature):
                all_frames_data.append((feature_id, i_frame, f_minx, f_miny, f_maxx, f_maxy, world.crs.to_string()))
